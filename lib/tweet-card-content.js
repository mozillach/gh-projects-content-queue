...
const retweetURLPattern = /https:\/\/(?:www\.)?twitter\.com\/[^\/]+\/tweets\/([0-9]+)\/?/;
...
class TweetCardContent {
     ...
     */
    getSection(title) {
        let rawSection = this.raw.split("##").find((s) => s.startsWith(title)).substr(title.length + 1);
...
`);
     ...
     */
    get tweet() {
        return this.getContent(TweetCardContent.TWEET_CONTENT);
    }
     ...
     *
     * @returns {string} Error with the card. Empty if there is no error.
     */
    getCardError() {
        if(!this.hasSection(TweetCardContent.TWEET_CONTENT) && !this.hasSection(TweetCardContent.RETWEET)) {
            return `Missing a section containing information on what to tweet. Please either add a "${TweetCardContent.TWEET_CONTENT}" or "${TweetCardContent.RETWEET}" section.`;
        }

        if(this.isRetweet) {
            if(!retweetURLPattern.test(this.getSection(TweetCardContent.RETWEET))) {
                return "URL for tweet to retweet is not valid. Please use the tweet permalink.";
            }
            else if(/*TODO tweet accessible? */) {
                return "Cannot retweet the given tweet because it does not exist or is private.";
            }
        }
        else {
            const charCount = twitter.getTweetLength(this.tweet);
            if(charCount > 140) {
                return `Tweet is too long with a character count of ${charCount}. Please shorten the tweet text to fit into 140 characters.`;
            }
        }
        ...
        if(this.hasSection(TweetCardContent.SCHEDULED)) {
            ...
            if(due < Date.now()) {
                return "Date is in the past. Please schedule tweet for some point in the future.";
            }
        }

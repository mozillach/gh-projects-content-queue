...
//TODO add infrastructure to reply to a tweet.
...
class TweetCardContent {
     ...
     */
    static createCard(meta, isRetweet = false, dueDate, dateFormat, replyTo) {
        ...
        if(dueDate && dateFormat) {
            card += createSection(TweetCardContent.SCHEDULED, ScheduledDate.format(dueDate, dateFormat));
        }
     ...
     */
    getSection(title) {
        let rawSection = this.raw.split("##").find((s) => s.startsWith(" "+title)).trim().substr(title.length);
        while(rawSection.startsWith("\n") || rawSection.startsWith("\r")) {
            rawSection = rawSection.substr(1);
        }
        while(rawSection.endsWith("\n")  || rawSection.endsWith("\r")) {
            rawSection = rawSection.substr(0, rawSection.length - 1);
        }
        return rawSection;
    }
    ...
    getCardError() {
        ...
        if(this.hasSection(TweetCardContent.SCHEDULED)) {
            ...
            } else if(due.getTime() < Date.now() - 60000) {

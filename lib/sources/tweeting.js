...
class TweetingSource extends Source {
        ...
        Promise.all([
            ...
            for(const card of Object.values(cards)) {
                if(!card.content.hasSection(TwitterFormatter.RETWEET)) {
                    ...
                    if(tweet) {
                        this.cardTweeted(card, `https://twitter.com/${tweet.user.screen_name}/status/${tweet.id_str}`, target);
                    }
                }
                else {
                    ...
                    if(didRetweet) {
                        this.cardTweeted(card, undefined, target);
                    }
                }
            }
        });
     ...
     *
     * @param {module:card.Card} card - Card to mark as tweeted.
    ...
    async tweet(card, target) {
        try {
            ...
            this.cardTweeted(card, url, target);
        }
        catch(e) {

...
class TweetingSource extends Source {
        ...
        Promise.all([
            ...
            for(const card of Object.values(cards)) {
                if(!card.content.isRetweet) {
                    const [ content ] = TwitterAccount.getMediaAndContent(card.content);
                    ...
                    const retweetID = TwitterAccount.getTweetIDFromURL(card.content.tweetToRetweet);
    ...
    async separateContentAndMedia(card) {
        const [ content, media ] = TwitterAccount.getMediaAndContent(card.content.tweet);

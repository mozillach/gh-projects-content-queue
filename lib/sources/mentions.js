...
class MentionsSource extends Source {
    ...
    constructor(repo, twitterAccount, config) {
        ...
        this.squad = new RotatingList(config.squad);
...
${MentionsSource.formatTweet(tweet)}`, false, null, config, tweetPermalink);

            const card = await repo.createCard(`Mention by ${tweet.user.screen_name}`, issueText.toString(), reactionColumn, "top").catch(console.error);
            card.assign(this.squad.getNext());
        });

...
class MentionsSource extends Source {
    ...
    constructor(repo, twitterAccount) {
        ...
        twitterAccount.on("mention", async (tweet) => {
            const reactionColumn = (await repo.columns.getData())[repo.columnIds.reactions];
            ...
            const openIssues = await repo.board.issues.issues.getData();
                ...
                if(issue.content.includes(TweetCardContent.REPLY_TO)) {
                    ...
                    if(content.isReply && content.replyTo == tweetPermalink) {
                        ...
                        if(!reactionColumn.hasCard(issue.id)) {
                            repo.board.addCard(issue, reactionColumn);
                        }
                        return;
                    }
                }
...
> ${tweet.text}`, false, null, null, tweetPermalink);
            ...
            repo.board.createCard(`Mention by ${tweet.user.screen_name}`, issueText.toString(), reactionColumn, "top").catch(console.error);

...
class Repository {
    ...
    constructor(githubClient, config) {
        ...
        p.push(this.hasFile("README.md").then((exists) => {
            if(!exists) {
                return this.addReadme();
            }
        }).then(() => {
            return this.hasFile("ISSUE_TEMPLATE.md");
        }).then((exists) => {
            if(!exists) {
                return this.addIssueTemplate();
            }
        }));

        p.push(this.ensureLabels());

        this.ready = Promise.all(p);
    }
    ...
    hasRequiredPermissions() {
        return this.githubClient("misc", "getRateLimit", {}).then((res) => {
            //TODO check X-OAuth-Scopes
            const scopes = res.meta["X-OAuth-Scopes"].split(",");
            return scopes.includes("repo_read") && scopes.includes("repo_write");
        });
    }

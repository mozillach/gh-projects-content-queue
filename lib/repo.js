...
class Repository {
     ...
     * @param {string} boardList - Markdown list of boards.
     */
    ...
    async hasRequiredPermissions() {
        const [
            ...
            this.githubClient.rateLimit.get({}),
            this.belongsToUser()
        ]);
     ...
     */
    hasFile(path) {
        return this.githubClient.repos.getContents({
    ...
    addFile(path, content, commit = "Setting up content queue.") {
        return this.githubClient.repos.createOrUpdateFile({
            owner: this.config.owner,
            repo: this.config.repo,
            path,
            message: commit,
...
${sections.join("\n\n")}`;
    ...
    async getUsersInTeam(team) {
        ...
        const opts = this.githubClient.teams.list.endpoint.merge({
            org: this.config.owner
        });
        const teams = await this.githubClient.paginate(opts);
        for(const t of teams) {
            if(t.name === team) {
                const memberOpts = this.githubClient.teams.listMembers.endpoint.merge({
                    team_id: t.id
                });
                const members = await this.githubClient.paginate(memberOpts);

...
class Issues extends EventEmitter {
    ...
    getIssueInfo(apiData) {
        return {
            ...
            asignee: apiData.assignee.login,
            labels: apiData.labels.map((l) => l.name),
    ...
    fetchOpenIssues() {
        return this.githubClient("issues", "getForRepo", {
            ...
            for(const issue in issues) {
                visitedIds.push(issue.id);
                if(!this.issues.has(issue.id)) {
                    this.issues.set(issue.id, new Issue(this.githubClient, this.getIssueInfo(issue)));
                    this.emit("opened", this.issues.get(issue.id));
                }
                else if(Date.parse(issue.updated_at) > this.issues.get(issue.id).lastUpdate) {
                    const issueModel = this.issues.get(issue.id);

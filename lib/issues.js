...
class Issues extends EventEmitter {
    ...
    createIssue(issue) {
        if(!(issue instanceof Issue)) {
            return Promise.reject("Issue must be an instance of Issue");
        }
        return this.githubClient("issues", "create", Object.assign({
            repo: this.config.repo,
            owner: this.config.owner
        }, issue.toAPIObject())).then((res) => {
            issue.update(this.getIssueInfo(res));
            return issue;
        });
    }
}

...
class ScheduledDate extends Date {
    ...
    constructor(date, config) {
        if(ScheduledDate.isValid(date)
           ...
           && typeof config.schedulingTime.timezone == "number"
           && ScheduledDate.matchesPattern(date, config.schedulingTime.format)) {
            ...
            ScheduledDate.adjustTimezone(this, config.schedulingTime.timezone);
        }
        else {
     ...
     * @returns {undefined}
     */
    static adjustTimezone(date, timezone, direction = false) {
        const localTimezone = date.getTimezoneOffset() / 60;
        let difference = timezone + localTimezone;
        if(direction) {
            difference = -localTimezone - timezone;
        }
        // Compensate for timezone difference if host timezone doesn't match
        // the one in the config.
        if(difference != 0) {
            let time = date.getTime();
            time -= Math.floor(difference * 3600000);
            date.setTime(time);
        }
    }
     ...
     */
    static formatDate(date, pattern, timezone) {
        if(date instanceof Date) {
            const dateCopy = new Date(date);
            if(timezone) {
                ScheduledDate.adjustTimezone(dateCopy, timezone, true);
            }
            return pattern
     ...
     */
    get valid() {
        return ScheduledDate.isValid(this.rawDate) && ScheduledDate.matchesPattern(this.rawDate, this.config.format);
    }
}

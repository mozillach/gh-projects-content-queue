...
const TwitterValidator = require("../validators/twitter");
...
class TwitterAccount extends ContentAccount {
    ...
    async separateContentAndMedia(card) {
        const contentSection = self(this).GetContentSection(card);
        const [ content, media ] = TwitterAccount.getMediaAndContent(contentSection);

...
class AccountManager {
    ...
    /**
     * @param {Set} [filterSet] - Set of account names to allow.
     * @return {ContentAccount[]} Array of content accounts.
     */
    getContentAccounts(filterSet) {
        const accounts = [];
        for(const type in this.accounts) {
            if(this.accounts.hasOwnProperty(type)) {
                const accountType = this.accounts[type];
                for(const name in accountType) {
                    if(accountType.hasOwnProperty(name) && accountType[name] instanceof ContentAccount && (!filterSet || filterSet.has(name))) {

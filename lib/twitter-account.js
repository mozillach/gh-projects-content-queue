 ...
 */
function getMentions(lastMention) {
    const args = {};
    if(lastMention !== null) {
...
class TwitterAccount extends DataStoreHolder {
    ...
    constructor(twitterClient) {
        ...
        this.ready = this.checkLogin().then(() => {
             ...
             * @type {number}
             */
            this.updateInterval = setInterval(() => this.update().catch(console.error), 60000);
        }).catch((e) => {
    ...
    static getRemainingChars(content) {
        const [ pureTweet ] = TwitterAccount.getGetMediaAndContent(content);
    ...
    static getGetMediaAndContent(tweet) {
        if(tweet.search(/!\[[^\]]*\]\([^)]+\)/) !== -1) {
            const media = [];
            const pureTweet = tweet.replace(/!\[[^\]]*\]\(([^)]+)\)/, (match, url) => {
                media.push(url);
                return '';
            });
            if(media.length > 4) {

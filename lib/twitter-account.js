 ...
 *
 * @event TwitterAccount#mention
 * @type {Object}
 */

class TwitterAccount extends EventEmitter {
    constructor(twitterClient) {
        super();
        this._twitterClient = twitterClient;
        this.lastMention = null;
        this.ready = this.checkLogin().then(() => {
            setTimeout(this.getMentions.bind(this), 60000);
        }).catch((e) => {
            console.error(e);
            throw e;
        });
    ...
    /**
     * @fires TwitterAccount#mention
     * @async
     * @returns {undefined}
     */
    getMentions() {
        const args = {};
        if(this.lastMention !== null) {
            args.since_id = this.lastMention;
        }
        return this._twitterClient.get('statuses/mentions_timeline', args).then((res) => {
            if(res.length > 0) {
                this.lastMention = res[0].id;

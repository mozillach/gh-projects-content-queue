...
function fetchColumns(oldColumns = {}) {
    return this.getBoardID().then((id) => {
        const opts = this.githubClient.projects.listColumns.endpoint.merge({
            project_id: id
        });
        return this.githubClient.paginate(opts);
    }).then((res) => {
        const columns = {};
...
class Board extends DataStoreHolder {
     ...
     * @param {module:repo~Repository} repo - GH repo this board is in.
     * @fires module:board.Board#opened
     */
    constructor(githubClient, config, repo) {
        super({
    ...
    getBoardID() {
        if(!this.id) {
            return this.githubClient.projects.listForRepo({
     ...
     */
    boardExists() {
        return this.getBoardID().then(() => true).catch((e) => {
     ...
     */
    createBoard() {
        return this.githubClient.projects.createForRepo({
            owner: this.config.owner,
            repo: this.config.repo,
    ...
    async setup() {
        ...
        this.repo.issues.on("updated", async (issue) => {
            const card = Array.from(this.cards.values()).find((c) => c.issue.id === issue.id);
            if(card) {
                card.updateContent();
            }
        });
        ...
        await Promise.all([
            this.repo.issues.issues.then(addIssues),
            this.repo.issues.closedIssues.then(addIssues)
        ]);
        this.cards.isReady = true;
    }
            ...
            for(const issue of issues.values()) {
                for(const column of columns) {
                    if(column && (await column.hasIssue(issue.number))) {
                        await this.addCard(issue, column, true);
                    }
                }
            }
        }
    ...
    async moveCardToColumn(card, column, localOnly = false, insertionPoint = "bottom") {
        if(!localOnly) {
            await this.githubClient.projects.moveCard({
                card_id: card.id,
    ...
    async createCard(title, text, column, position) {
        const issue = await this.repo.issues.createIssue(title, text);

...
class Board extends EventEmitter {
    ...
    createCard(card, column) {
        ...
        if(!column) {
            column = this.columns[this.columnIds.reactions];
        }
     ...
     * @returns {undefined}
     */
    addCard(issue, column) {
        if(!column) {
            column = this.columns[this.columnIds.ideas];
        }
     ...
     * @todo insertion point is bugged/de-synced between the github API and the lib.
     * @returns {undefined}
     */
     ...
     *
     * @returns {undefined}
     * @todo not convinced this is working, need to check.
     */
    syncCardsToColumns() {
    ...
    cardTweeted(card, url) {
        ...
        return Promise.all([
            card.issue.close(),
            this.moveCardToColumn(card, this.columns[this.columnIds.tweeted], false, "top"),

...
//TODO also keep reference of non-default columns for checking of those issues.
...
class Board {
    ...
    constructor(githubClient, config) {
        ...
        this.updateInterval = setInterval(() => {
            ...
            //TODO move cards based on the results from this?
        }, 60000);
        ...
        this.setup().then(() => {
            ...
            this.issues.on("opened", (issue) => {
                for(const column of Object.values(this.columns)) {
                    if(column && column.hasIssue(issue.number)) {
                        column.addCard(new TweetCard(issue, config));
                        return;
                    }
                }
                if(this.columnIds.ideas !== 0) {
                    this.columns[this.columnIds.ideas].addCard(new TweetCard(issue, config));
                }
            });
        ...
        return co(function* () {
            for(const column in columnOrder) {
                if(column in this.columnIds && this.columnIds[column] !== 0) {
                    ...
                    yield this.columns[this.columnIds[column]].move(position);
                }
            }
        }.bind(this));
    }

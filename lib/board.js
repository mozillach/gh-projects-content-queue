 ...
 * @async
 * @this Board
 * @returns {Object.<string,Column>} New column object.
 */
    ...
    return this.getBoardID().then((id) => {
        ...
        for(const c in res) {
            ...
            if(column.id in oldColumns) {
                ...
                columns[column.id] = new Column(this.githubClient, column.id);
            }
        ...
        return Promise.all(updates).then(() => columns);
    });
}
...
class Board extends DataStoreHolder {
     ...
     * @listens Issues#updated
     * @listens Issues#opened
     * @listens Issues#closed
     * @fires Board#opened
     * @constructs
     */
    constructor(githubClient, config) {
        ...
        this.setup().then(() => {
            ...
            this.issues.on("opened", async (issue) => {
                const columns = Object.values(await this.columns.getData());
            ...
            this.issues.on("updated", async (issue) => {
                const columns = Object.values(await this.columns.getData());
                for(const column of columns) {
            ...
            this.issues.on("closed", async (issue) => {
                const columns = Object.values(await this.columns.getData());
                for(const column of columns) {
                    //TODO generalize this into columns that are not to be worked on provided by sources
                    if(column && column.hasCard(issue.id) && column.id != this.columnIds.tweeted) {
    ...
    async createColumn(name) {
        ...
        const columns = await this.columns.getData();
        columns[res.id] = res;
    }
    ...
    async arrangeColumns() {
        for(const column in columnOrder) {
            if(column in this.columnIds && this.columnIds[column] !== 0) {
                ...
                const columns = this.columns.getData();
                await columns[this.columnIds[column]].move(position);
            }
        }
    }
    ...
    async createCard(title, text, column, position) {
        if(!column) {
            const columns = await this.columns.getData();
            column = columns[this.columnIds.reactions];
        }

        const issue = await this.issues.createIssue(title, text);
        const card = await this.addCard(issue, column);
        if(position) {
     ...
     */
    async addCard(issue, column) {
        if(!column) {
            const columns = await this.columns.getData();
            column = columns[this.columnIds.ideas];
        }

        return column.addCard(new TweetCard(issue, this.config));
    }
    ...
    async cardTweeted(card, url) {
        ...
        return Promise.all([
            card.issue.close(),
            this.moveCardToColumn(card, columns[this.columnIds.tweeted], false, "top"),

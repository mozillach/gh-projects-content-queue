...
class MastodonValidator extends Validator {
    ...
    validate(cardContent) {
        ...
        if(!MastodonFormatter.CONTENT_SECTIONS.some((section) => cardContent.hasSection(section))) {
            errors.push(`Missing a section containing information on what to tweet. Please either add a "${MastodonFormatter.CONTENT}" or "${MastodonFormatter.REBLOG}" section.`);
        }
        else {

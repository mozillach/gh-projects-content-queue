...
/**
 * Validates a board configuration.
 *
 * @param {?} boardConfig - Config for a board.
 * @param {string} repo - Name of the repo.
 * @throws When the config is invalid.
 * @returns {undefined}
 */
const validateBoardConfig = (boardConfig, repo) => {
    if(!("name" in boardConfig)) {
        throw 'Missing "name" in board config for ' + repo;
    }

    if(!("columns" in boardConfig)) {
        throw 'Missing "columns" in board config for ' + repo;
    }
};
...
const validateProjectConfig = (projectConfig) => {
    ...
    if(!("board" in projectConfig)) {
        throw 'Missing a board configuration in "board" for ' + projectConfig.repo;
    }

    if(!isObject(projectConfig.board)) {
        throw "Board configuration has an invalid type for " + projectConfig.repo;
    }

    validateBoardConfig(projectConfig.board, projectConfig.repo);
};

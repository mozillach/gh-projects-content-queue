...
/**
 * @this module:column.Column
 * @param {external:Set} [cards=new Set()] - Cards that are alread in the column.
 * @returns {external:Set} Cards currently in the column.
 */
async function syncCards(cards = new Set()) {
    const issues = await this.issues;
    await Promise.all(Array.from(cards.values()).map(async (card) => {
    ...
    for(const i in issues) {
        const card = issues[i];
        if(!this.hasGithubCardId(card.id)) {
            const cardInstance = this.allCards.get(card.id);
            if(cardInstance) {
                cardInstance.column = this;
                cards.add(cardInstance.id);
            }
            //TODO request card or something?
        }
    }
...
class Column extends DataStoreHolder {
     ...
     * @param {external:Map} cards - All cards on the board.
     ...
     * @param {external:Map} - All cards on the board.
     */

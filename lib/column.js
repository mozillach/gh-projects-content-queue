...
async function fetchCards() {
    const opts = this.githubClient.projects.listCards.endpoint.merge({
        column_id: this.id
    });
    const response = await this.githubClient.paginate(opts);
...
class Column extends DataStoreHolder {
    ...
    static create(client, projectId, name, cards) {
        return client.projects.createColumn({
     ...
     */
    move(position) {
        return this.githubClient.projects.moveColumn({
            column_id: this.id,
            position
        });
    }
    ...
    async addCard(card, localOnly = false) {
        if(card.column !== this) {
            if(!(await this.hasIssue(card.issue.number))) {
                if(!localOnly) {
                    const { data: res } = await this.githubClient.projects.createCard({
                        column_id: this.id,
    ...
    async removeCard(card, localOnly = false, keepCard = false) {
        if(!localOnly) {
            await this.githubClient.projects.deleteCard({
                card_id: card.id
            });
        }
        const cards = await this.cards;
    ...
    moveCard(card, position) {
        return this.githubClient.projects.moveCard({
            card_id: card.id,
            position
        });
    }
